...
- API 已处理 `SIGINT`/`SIGTERM`，停止接新 HTTP 连接，并最多等待 25 秒完成 in-flight requests。
...
- Helm chart 已提供 `values.production.example.yaml`，集中展示生产部署常用的 image、config、resources、autoscaling、NetworkPolicy、worker concurrency 和外部 Secret 边界。
- 已新增 backup/restore runbook，明确 Soniq Postgres、object storage、Temporal、Kubernetes Secret 的备份边界、恢复顺序和恢复后验证。
