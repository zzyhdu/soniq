...
object storage PutObject for normalized.wav + recording_normalized_audios upsert
  ↓
configured transcription provider receives a temporary normalized audio path and, when object storage supports it, a presigned normalized audio URL
  ↓
...
With `STORAGE_PROVIDER=local`, objects are stored under `LOCAL_STORAGE_PATH`.
...
temporary local files, while URL-capable ASR providers such as DashScope can use
the presigned normalized audio URL instead of Base64 payloads. The fake
