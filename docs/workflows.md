...
Current user-facing delete is a soft delete:
...
- Transcript, summary, mind map, audio probe, normalized audio metadata, and storage artifacts are retained until a future Trash purge flow.
- Existing `ON DELETE CASCADE` constraints remain as a database integrity backstop for future hard delete, but soft delete is coordinated in application code.
